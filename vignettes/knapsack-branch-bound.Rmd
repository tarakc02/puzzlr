...
I can work out a recursive solution. Assume we know the value of `min_wt` for numbers less than `n`. Then I'm left looking at the nth item. The solution to `min_wt(n, v)` either includes the nth item, in which case:

```{r, eval = FALSE}
min_wt(n, v) == weight_nth_item + min_wt(n - 1, v - value_nth_item)
```
...
There's still a problem: I'm re-calculating `min_wt` over and over for the same inputs. Much like calculating the Fibonacci sequence...
...
# solve_knapsack(ks, bounds = greedy, search_strategy = best_first) %>%
#     stream_filter(function(x) n_items(x$problem) == 0) %>% "["(1)
# solve_knapsack(ks, bounds = function(x) approxi_bounds(x, .3), 
