...
There's still a problem: I'm re-calculating `mincap` over and over for the same inputs. To get the value of `mincap(n, v)` I need to calculate `mincap(n - 1, v)` as well as `mincap(n - 1, v - value[n])` and so on. This is an instance where caching, or memoization, can make a big difference. Since `mincap` has two integer arguments and an integer output, I can use a matrix to store already-calculated values -- looking up a single value in a matrix can be done very quickly:
...
exact_solution <- function(ks) {
    ...
    list(lower_bound = k, upper_bound = k,
         solution = take_items(ks, items$id[selected]))
}
...
problems <- expand.grid(
    n = seq(15, 30, by = 5),
    type = c("weakly_correlated_instance",
             "strongly_correlated_instance",
             "subset_sum_instance"),
    stringsAsFactors = FALSE) %>% tbl_df %>%
    mutate(arglist = map(n, ~list(n = .x)),
           instance = map2(type, arglist, ~do.call(.x, args = .y))) %>%
    select(-arglist)
```

I can use `purrr` to generate solutions for each problem. I

```{r}
solutions <- problems %>%
    mutate(fptas = map(instance, solve_knapsack,
                       bounds = function(x) fptas(x, .3),
                       search_strategy = best_first),
           greedy = map(instance, solve_knapsack,
                        bounds = greedy,
                        search_strategy = best_first))
solutions
```

I'll create a couple of helper functions to make the rest of the code clearer -- `find_opt` will find the index of the optimal solution, and `to_df` will extract some useful metrics from the solving process. These mirror what we did to find a solution in the first section. 

```{r}
find_opt <- function(s) 
    as.integer(stream_which(s, 
                            function(x) n_items(x$problem) == 0)[1])

to_df <- function(s, until) {
    data_frame(
...
I next use `find_opt` to find the index of the optimal solution. For some reason, `purrr` functions errored on the next bit of code, but the `*apply` functions worked just fine.

```{r}
solutions <- solutions %>%
    mutate(fptas_i = vapply(fptas, find_opt, FUN.VALUE = integer(1)),
           greedy_i = vapply(greedy, find_opt, FUN.VALUE = integer(1)))
```

Notice that the two solution strategies required the same number of iterations to get a provably optimal solution. That's because to prove optimality, I need to show that my solution is better than the upper bound of any other solution path. `fptas` improved our lower bounds, but not the upper bounds (exercise for another day: implement stronger upper bounds). However, the improved lower bounds mean we are able to achieve a much higher "percent of known upper bound" value much earlier in the process. If we only needed a solution that was provably close to optimal, without necessarily being optimal, the better lower bounds `fptas` would allow us to stop eralier. 

```{r}
solutions <- solutions %>%
    mutate(fptas_solution = map2(fptas, fptas_i, ~to_df(.x, .y)),
           greedy_solution = map2(greedy, greedy_i, ~to_df(.x, .y)))

solutions %>% 
    select(n, type, fptas_solution, greedy_solution) %>%
    gather(solution_type, solution, -n, -type) %>%
    unnest %>%
    mutate(pct_of_upper_bound = best_seen / upper_bound) %>%
    # 0-index and normalize the time to [0,1]
    mutate(timestamp = timestamp - 1) %>%
    group_by(n, type, solution_type) %>% 
    mutate(timestamp = timestamp / max(timestamp)) %>%
    ggplot(aes(x = timestamp, y = pct_of_upper_bound, colour = solution_type)) +
    geom_line() +
    facet_grid(type ~ n, scales = "free")
```

As expected, given the same number of iterations, `fptas`-bounding gives us a better non(provably)-optimal solution up until the moment both strategies are able to prove optimality.
