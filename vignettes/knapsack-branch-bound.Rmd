...
greedy <- function(ks) {
    solution_so_far <- ks
    current_item <- next_item(ks)
    while (!is.null(current_item) && 
           current_item$weight <= capacity(solution_so_far)) {
        solution_so_far <- take(solution_so_far)
        current_item <- next_item(solution_so_far)
    }
    
    lower_bound <- total_value(solution_so_far)
    upper_bound <- lower_bound
    if (capacity(solution_so_far) > 0 && n_items(solution_so_far) > 1) {
        partial_amount <- capacity(solution_so_far) / current_item$weight
        upper_bound <- upper_bound + (partial_amount * current_item$value)
    }
    
    list(lower_bound = lower_bound,
         upper_bound = upper_bound,
         solution = solution_so_far)
}
...
approxi_bounds <- function(ks, epsilon) {
    ...
    approximate_solution <- taken_items(dynamic(new_ks)$solution)
    approximate_solution <- approximate_solution$id
    take_items(ks, approximate_solution)
}
```
...
system.time(dp(ks))
system.time(dynamic(ks))
system.time(approxi_bounds(ks, epsilon = .9))
system.time(greedy(ks))
```
